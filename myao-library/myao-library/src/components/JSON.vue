...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <p>We operate in: {{ countries.join(', ') }}</p>
...
<section class="lab-section">
  ...
  <div v-for="author in authors" :key="author.name">
    <p
      :style="isHighlighted ? { color: highlightColor, fontSize: highlightSize + 'px' } : {}"
    >
      {{ author.name }} ({{ author.birthYear }})
    </p>
  </div>
...
const storeTypes = Object.entries(bookstores.storeTypes);
const openingHours = Object.entries(bookstores.openingHours);
const countries = bookstores.countries;
const topSellers = bookstores.topSellers;
const showMessage = ref(false);

const highlightColor = ref('blue')
const highlightSize = ref(20)
